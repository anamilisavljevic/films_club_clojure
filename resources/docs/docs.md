...
The `home-routes` handler in the `films-club-clojure.routes.home` namespace
...
The `home-page` function will in turn call the `films-club-clojure.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `films-club-clojure.handler` namespace:
...
Request middleware functions are located under the `films_club_clojure.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `films-club-clojure.dev-middleware` namespace found in
